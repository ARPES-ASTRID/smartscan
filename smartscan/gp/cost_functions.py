...
def movement_cost(
    ...
    if prev_points.shape[0] > 0:
        ...
        if np.any(all_distances < min_distance):
            ...
            if verbose:
                logger.debug(
                    f"Point {np.asarray(x).ravel()} close to previous point {prev_points[idx]}: d={all_distances[idx]}"
                )
            return [1_000_000]
    ...
    if verbose:
        logger.debug(
            f"Distance: {distance:.2f} um, Time: {time:.2f} s"
...
def compute_costs(
    ...
    cost = []
    for xx in x:
        movcost: float = movement_cost(
            origin, xx, cost_func_params, verbose=verbose, logger=logger
        )
        cost.append(movcost)
...
def weighted_manhattan_distance(
    ...
    assert (
        len(a) == len(b) == len(weights)
    ), "a, b and weights must have the same length"
...
def manhattan_cost_function(
    ...
    unrecognized = [
        ...
        if k not in ["speed", "dwell_time", "dead_time", "point_to_um", "weight"]
    ]
    ...
    for i in range(x.shape[0]):
        distance = (
            weighted_manhattan_distance(origin, x[i, :], weight / speed) * point_to_um
        )
    ...
    if prev_points.shape[0] > 0 and n_dim == 2:
        ...
        for xx in x:
            ...
            if np.any(all_distances < min_distance):
                idx = np.argmin(all_distances - min_distance)
                logger.warning(
                    f"Point {np.asarray(xx).ravel()} close to previous point {prev_points[idx]}: d={all_distances[idx]}"
                )
