...
@pytest.fixture
def event_loop() -> asyncio.Generator[asyncio.AbstractEventLoop, asyncio.Any, None]:
...
@pytest.fixture
def mock_client_connection(event_loop) -> asyncio.Generator[tuple[mock.Mock, mock.Mock], asyncio.Any, None]:
...
@pytest.mark.asyncio
async def test_handle_client(mock_client_connection) -> None:
...
def test_run_server(event_loop) -> None:
    server = TCPServer("localhost", 12345)
